...
export class AllExceptionFilter implements ExceptionFilter {
  ...
  @SentryExceptionCaptured()
    ...
    if (
      ...
      const detailMessage =
        'detail' in exception ? (exception as { detail: string }).detail : '';
    ...
    if (!isBaseExceptionFormat) {
      if (Array.isArray(message)) {
        ...
        const messageObj = message as { error?: string; message?: string };
        message = new BaseExceptionResponse(
          isObject && messageObj.error
            ? toCamelCase(messageObj.error)
            : 'internalServerError',
